...
## \brief A class for manually driving a robot using arrow keys.
#
#  \details This class allows the user to manually control a robot's movement using the arrow keys.
#  @param self The object pointer.
class ManualDrive:

    ## \brief Initializes an instance of the ManualDrive class.
    #
    #  \details This method initializes an instance of the ManualDrive class and creates a CvBridge object to convert sensor_msgs/Image type to cv2 image.
    def __init__(self):
    ...
    ## \brief Callback function to process key events and control robot's movement.
    #
    #  \details This method processes key events and controls a robot's movement based on the key pressed.
    ...
    def callback(self,data):
        ...
        if key == 82: self.CarActions.move_forward(), print("straight")

        # turn left when left arrow key is pressed
        elif key == 81: self.CarActions.turn_left(), print("left")

        # turn right when right arrow key is pressed
        elif key == 83: self.CarActions.turn_right(), print("right")

        # move backward when down arrow key is pressed
        elif key == 84: self.CarActions.move_backward(), print("back")

        # stop when spacebar is pressed
        elif key == ord(" "): self.CarActions.stop(), print("stop")
